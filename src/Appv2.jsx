...
const STD_WIDTHS  = { base:[9,12,15,18,21,24,27,30,33,36,42,48], upper:[9,12,15,18,21,24,27,30,33,36,42,48], tall:[18,24,30,36], island:[36,42,48,54,60,72,84,96], vanity:[18,21,24,27,30,33,36,42,48] };
const STD_HEIGHTS = { base:[30,31.5,34.5,36], upper:[12,15,18,24,30,36,42], tall:[72,84,90,96], island:[28.5,34.5,36,42], vanity:[28.5,30,31.5,32,34.5] };
const STD_DEPTHS  = { base:[12,15,18,21,24], upper:[12,13,14], tall:[12,18,24], island:[24,27,30,36], vanity:[18,21,24] };
...
const makeCab = (type, wall, all, wallW) => {
  const def = DEFS[type];
  const row = all.filter(c => c.wall===wall && DEFS[c.type]?.row===def.row);
  const nextX = row.length ? Math.max(...row.map(c=>c.x+c.w))+3 : 3;
  const safeX = wallW ? Math.min(nextX, wallW-def.w) : nextX;
  return { id:uid(), type, w:def.w, h:def.h, d:def.d, x:Math.max(0,safeX),
    ix:0, iy:0, wall:type==="island"?"Island":wall,
    material:"maple", doorStyle:"Shaker", finish:"Natural", notes:"", useStandard:true };
};
...
const Label = ({children,style}) => (
  <div style={{fontSize:11,fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase",color:T.muted,marginBottom:6,...style}}>{children}</div>
);
const IS_base = {background:"#fff",border:`1.5px solid ${T.border}`,borderRadius:6,padding:"8px 12px",fontSize:14,color:T.ink,outline:"none",width:"100%"};
const FInput = ({style,...props}) => (
  <input style={{...IS_base,...style}} onFocus={e=>e.target.style.borderColor=T.amber} onBlur={e=>e.target.style.borderColor=T.border} {...props}/>
);
const FSelect = ({style,children,...props}) => (
  <select style={{...IS_base,...style}} onFocus={e=>e.target.style.borderColor=T.amber} onBlur={e=>e.target.style.borderColor=T.border} {...props}>{children}</select>
);
const Btn = ({children,variant="primary",style,...props}) => {
  const v = {
    primary:{background:T.amber,color:"#fff",border:"none"},
  ...
  return (
    <button style={{padding:"10px 20px",borderRadius:7,fontSize:14,fontWeight:600,display:"inline-flex",alignItems:"center",gap:8,transition:"all 0.15s",...v,...style}}
...
function Dropzone({label,onFile,preview}) {
  ...
  return (
    <div onClick={()=>inp.current.click()} onDragOver={e=>{e.preventDefault();setDrag(true);}} onDragLeave={()=>setDrag(false)}
      ...
      {preview
        ...
        :<div style={{textAlign:"center",padding:"28px 20px",color:T.faint}}><div style={{fontSize:28,marginBottom:8}}>📷</div><div style={{fontSize:14,fontWeight:500,color:T.muted,marginBottom:4}}>{label}</div><div style={{fontSize:12,color:T.faint}}>Click or drag & drop · JPG or PNG</div></div>
      }
    </div>
  );
}
...
function RoomSetupView({room,setRoom,activeWalls,setActiveWalls,onNext}) {
  ...
  const toggleWall=w=>setActiveWalls(prev=>prev.includes(w)?prev.filter(x=>x!==w):[...prev,w]);
  ...
  const analyze=async()=>{
    ...
    try {
      const txt=await callClaude([{role:"user",content:[
        ...
        {type:"text",text:`Analyze this image and extract kitchen room dimensions. It may be a photo, hand-drawn sketch, or floor plan.

Return ONLY valid JSON, no markdown:
{"width":<inches>,"depth":<inches>,"height":<inches, default 96>,"confidence":"high|medium|low","notes":"<one sentence>","walls":["South","West"],"features":["<feature>"],"suggestedLayout":"Galley|L-Shape|U-Shape|G-Shape|Island Kitchen","existingObstacles":["<obstacle>"]}

Rules:
- Convert feet to inches (×12). E.g. 7' = 84", 8' = 96"
- "walls" = only walls that have space for cabinets. L-shape = 2 walls, Galley = 2 opposite walls, U-shape = 3 walls
- If unclear, use ["South","West"]
- Typical kitchens: 96–192" wide, 96–168" deep`}
      ]}]);
  ...
  return (
    <div style={{flex:1,overflowY:"auto",padding:"36px 48px",maxWidth:920}}>
      <div className="fade-up">
        ...
        <p style={{fontSize:15,color:T.muted,marginBottom:32}}>Upload a photo of the space and a sketch or blueprint. The AI reads dimensions and detects which walls have cabinet space.</p>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:28}}>
          <div>
            ...
            <p style={{fontSize:12,color:T.faint,marginTop:6}}>Helps understand windows, doors, and layout</p>
          </div>
          <div>
            <Label>Blueprint, sketch, or floor plan <span style={{color:T.amber}}>★ recommended</span></Label>
            <Dropzone label="Upload blueprint or sketch" onFile={f=>{setBlueprint(f);setAnalysis(null);}} preview={blueprint?.dataUrl}/>
            <p style={{fontSize:12,color:T.faint,marginTop:6}}>Even a hand-drawn sketch with measurements works</p>
          </div>
        </div>
        ...
        {analysis&&(
          <Card style={{marginBottom:24,borderColor:T.amber}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
              {analysis.features?.length>0&&<div><Label>Detected features</Label>{analysis.features.map((f,i)=><div key={i} style={{fontSize:13,color:T.text,marginBottom:4}}>→ {f}</div>)}</div>}
              {analysis.existingObstacles?.length>0&&<div><Label>Obstacles</Label>{analysis.existingObstacles.map((o,i)=><div key={i} style={{fontSize:13,color:T.red,marginBottom:4}}>⚠ {o}</div>)}</div>}
            </div>
            ...
            {[["Room width","width","Wall to wall"],["Room depth","depth","Front to back"],["Ceiling height","height","Floor to ceiling"]].map(([l,k,hint])=>(
              <div key={k}>
                ...
                <div style={{display:"flex",gap:8,alignItems:"center"}}>
                  <FInput type="number" value={room[k]} style={{width:90}} onChange={e=>setRoom(r=>({...r,[k]:parseInt(e.target.value)||96}))}/>
        ...
        <Card style={{marginBottom:28}}>
          ...
          <p style={{fontSize:14,color:T.muted,marginBottom:16}}>Only select walls that actually exist with room for cabinets. Galley = 2 walls · L-shape = 2 walls · U-shape = 3 walls.</p>
          <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:16}}>
            {ALL_WALLS.map(w=>{
              const isOn=activeWalls.includes(w);
              return (
                <button key={w} onClick={()=>toggleWall(w)}
                  style={{padding:"10px 22px",borderRadius:8,fontSize:14,fontWeight:600,
                    background:isOn?T.amber:T.surface,border:`2px solid ${isOn?T.amber:T.border}`,
                    color:isOn?"#fff":T.muted,cursor:"pointer",transition:"all 0.15s"}}>
                  {isOn?"✓ ":""}{w} wall
                </button>
              );
            })}
          ...
          <div style={{marginTop:8}}>
            <Label style={{marginBottom:10}}>Room footprint preview</Label>
            <RoomFootprint room={room} activeWalls={activeWalls}/>
          </div>
        </Card>

        <Btn onClick={onNext} disabled={activeWalls.length===0} style={{fontSize:15,padding:"12px 28px"}}>
  ...
  return (
    <svg width={W+P*2} height={D+P*2} style={{display:"block",borderRadius:8,background:T.wallBg}}>
      ...
      {[["South",P+W/2,P+D+20,0],["North",P+W/2,P-12,0],["East",P+W+20,P+D/2,90],["West",P-20,P+D/2,-90]].map(([w,x,y,rot])=>(
        <text key={w} x={x} y={y} textAnchor="middle" fill={wc(w)} fontSize={11} fontFamily="DM Sans"
          fontWeight={activeWalls?.includes(w)?600:400} transform={rot?`rotate(${rot} ${x} ${y})`:undefined}>{w}</text>
      ))}
...
function RecommendationsView({room,activeWalls,onApply,onSkip}) {
  ...
  const generate=async()=>{
    ...
    try {
      ...
      const txt=await callClaude([{role:"user",content:
        `You are an NKBA-certified kitchen designer. Generate a professional cabinet layout.

Space: ${room.width}" wide × ${room.depth}" deep × ${room.height}" ceiling (${sqft} sq ft)
AVAILABLE WALLS: ${wallList} — ONLY place cabinets on these walls, never others.
...
STRICT RULES: Only use walls: ${wallList}. Never use any other wall. 10–16 cabinets total. Mix base, upper/wall, one tall pantry. Add island ONLY if room ≥120" wide AND ≥144" deep. x positions start at 3, never exceed wall width minus cabinet width.`
      }],2500);
  ...
  return (
    <div style={{flex:1,overflowY:"auto",padding:"36px 48px",maxWidth:920}}>
      <div className="fade-up">
        ...
        <div style={{display:"flex",gap:8,marginBottom:24,flexWrap:"wrap"}}>{activeWalls.map(w=><Badge key={w} color="amber">{w} wall</Badge>)}</div>

        {!recs&&(
          <div>
            <Btn onClick={generate} disabled={loading} style={{marginBottom:12}}>
              {loading?<><span style={{animation:"spin 0.8s linear infinite",display:"inline-block"}}>⟳</span> Generating layout…</>:"⚡ Generate AI recommendations"}
            </Btn>
        ...
        {recs&&(
          <div style={{display:"flex",flexDirection:"column",gap:20}}>
            ...
            {recs.tips?.length>0&&<Card><Label style={{marginBottom:12}}>Design tips</Label>{recs.tips.map((t,i)=><div key={i} style={{display:"flex",gap:12,marginBottom:10}}><span style={{color:T.amber,fontWeight:700,flexShrink:0}}>→</span><span style={{fontSize:14,color:T.text,lineHeight:1.65}}>{t}</span></div>)}</Card>}
...
function ElevationCanvas({cabs,wall,wallW,wallH,sel,onSel,onMove}) {
  ...
  return (
    <svg width={SW} height={SH} style={{display:"block",background:T.canvasBg,userSelect:"none",minWidth:SW,borderRadius:8}}
      ...
      {wallCabs.map(c=>{
        const isSel=c.id===sel,isLower=DEFS[c.type]?.row==="lower";
        ...
        return (
          <g key={c.id} style={{cursor:"grab"}} onPointerDown={e=>onPD(e,c.id)} onClick={e=>{e.stopPropagation();onSel(c.id);}}>
            <rect x={cx} y={cyy} width={cw} height={bodyH} fill={bf} stroke={isSel?T.amber:de} strokeWidth={isSel?2:1} rx={1}/>
      ...
      {wallCabs.length===0&&<text x={PL+wallW*ES/2} y={FY-wallH*ES/2+6} textAnchor="middle" fill="#C4B8A8" fontSize={14} fontFamily="DM Sans">← Add cabinets from the left panel</text>}
    </svg>
  );
}
...
function FloorPlanCanvas({cabs,room,sel,onSel,onMoveIsland}) {
  ...
  const getRect=c=>{
    ...
    if(c.wall==="Island"){
      const cx=clamp(L+room.width*FS/2-cw/2+(c.ix||0)*FS,L,R-cw);
      const cy=clamp(Tp+room.depth*FS/2-cd/2+(c.iy||0)*FS,Tp,B-cd);
      return{x:cx,y:cy,w:cw,h:cd};
    }
    switch(c.wall){
      ...
      {cabs.map(c=>{
        ...
        return (
          <g key={c.id} style={{cursor:isIsland?"grab":"pointer"}} onPointerDown={e=>onPD(e,c)} onClick={e=>{e.stopPropagation();onSel(c.id);}}>
            <rect x={r.x} y={r.y} width={r.w} height={r.h} fill={isIsland?"#C4A870":isUpper?"#D4CCBC":"#C8B898"} stroke={isSel?T.amber:T.borderDark} strokeWidth={isSel?2:1} strokeDasharray={isUpper?"3,2":"none"} rx={2}/>
            <text x={r.x+r.w/2} y={r.y+r.h/2+4} textAnchor="middle" fill={isSel?T.amber:T.muted} fontSize={10} fontFamily="DM Sans">{c.w}"</text>
  ...
  return (
    <div style={{width:240,background:T.sidebar,display:"flex",flexDirection:"column",flexShrink:0,overflowY:"auto"}}>
      <div style={{padding:"20px 16px",borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:activeWalls.length>2?"1fr 1fr":"1fr 1fr",gap:6}}>
          {activeWalls.map(w=>{
            const count=cabs.filter(c=>c.wall===w).length,isA=wall===w;
            return (
              <button key={w} onClick={()=>setWall(w)}
                style={{padding:"9px 8px",background:isA?T.amber:"rgba(255,255,255,0.06)",border:`1px solid ${isA?T.amber:"rgba(255,255,255,0.12)"}`,borderRadius:7,color:isA?"#fff":"rgba(255,255,255,0.55)",fontSize:13,fontWeight:600,cursor:"pointer",position:"relative",transition:"all 0.15s"}}>
                {w}
      ...
      <div style={{padding:"16px",borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
        ...
        {[["Width","width"],["Depth","depth"],["Height","height"]].map(([l,k])=>(
          <div key={k} style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
            ...
            <input type="number" value={room[k]} onChange={e=>setRoom(r=>({...r,[k]:parseInt(e.target.value)||96}))}
              style={{width:64,background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:5,padding:"5px 8px",color:"#FAF7F2",fontSize:13,outline:"none"}}/>
      ...
      <div style={{padding:"16px",flex:1}}>
        ...
        {Object.entries(DEFS).filter(([t])=>t!=="island").map(([type,def])=>(
          <button key={type} onClick={()=>onAdd(type)}
            style={{display:"block",width:"100%",padding:"10px 12px",marginBottom:6,background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,color:"rgba(255,255,255,0.8)",textAlign:"left",cursor:"pointer",transition:"all 0.15s"}}
        ...
        <div style={{marginTop:12,paddingTop:12,borderTop:"1px solid rgba(255,255,255,0.08)"}}>
          ...
          <button onClick={()=>onAdd("island")}
            style={{display:"block",width:"100%",padding:"10px 12px",background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,color:"rgba(255,255,255,0.8)",textAlign:"left",cursor:"pointer",transition:"all 0.15s"}}
            onMouseEnter={e=>{e.currentTarget.style.background="rgba(193,122,58,0.25)";e.currentTarget.style.borderColor=T.amber;}}
            onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,255,255,0.06)";e.currentTarget.style.borderColor="rgba(255,255,255,0.1)";}}>
...
function PropertiesPanel({c,update,del,activeWalls}) {
  ...
  const stdW=STD_WIDTHS[c.type]||[],stdH=STD_HEIGHTS[c.type]||[],stdD=STD_DEPTHS[c.type]||[];
  const isBase=DEFS[c.type]?.row==="lower";
  const hWarning=isBase&&(c.h<30||c.h>36)?(c.h<30?"Below standard — may be too low":"Above standard — countertop will be unusually high"):null;
  const IS={background:"#fff",border:`1.5px solid ${T.border}`,borderRadius:6,padding:"7px 10px",fontSize:14,color:T.ink,outline:"none",width:"100%"};

  const DimField=({label,k,stdList})=>(
    <div>
      <div style={{fontSize:11,color:T.faint,marginBottom:4,textAlign:"center"}}>{label}</div>
      {useStd&&stdList.length>0
        ?<select value={c[k]} onChange={e=>update(c.id,{[k]:parseFloat(e.target.value)})} style={{...IS,padding:"7px 4px",textAlign:"center"}}>
           {stdList.map(v=><option key={v} value={v}>{v}"</option>)}
         </select>
  ...
  return (
    <div style={{width:280,background:T.surface,borderLeft:`1px solid ${T.border}`,flexShrink:0,overflowY:"auto",padding:20}}>
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:14,padding:"10px 12px",background:T.bg,border:`1px solid ${T.border}`,borderRadius:8}}>
        <div>
          <div style={{fontSize:13,fontWeight:600,color:T.ink}}>{useStd?"Standard sizes":"Custom sizes"}</div>
          <div style={{fontSize:11,color:T.faint}}>{useStd?"Common dimensions":"Free entry"}</div>
        </div>
        <button onClick={()=>update(c.id,{useStandard:!useStd})}
          style={{width:44,height:24,borderRadius:12,border:"none",cursor:"pointer",background:useStd?T.amber:T.border,position:"relative",transition:"background 0.2s",flexShrink:0}}>
      ...
      <div style={{marginBottom:14}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
          <DimField label='W"' k="w" stdList={stdW}/>
          <DimField label='H"' k="h" stdList={stdH}/>
          <DimField label='D"' k="d" stdList={stdD}/>
        </div>
        {hWarning&&(
          <div style={{marginTop:8,padding:"8px 10px",background:"#FFF8E8",border:"1px solid #F0D080",borderRadius:6,fontSize:12,color:"#8B6020"}}>
            ⚠ {hWarning}<br/><span style={{color:T.faint}}>Standard base: 34.5" (36" with countertop)</span>
          </div>
        )}
      </div>
      ...
      {[["Material","material",Object.entries(MATS).map(([v,m])=>({v,l:m.label}))],["Door style","doorStyle",DOOR_STYLES]].map(([label,k,opts])=>(
        <div key={k} style={{marginBottom:12}}>
          <Label>{label}</Label>
          <select value={c[k]} onChange={e=>update(c.id,{[k]:e.target.value})} style={IS}>
            {opts.map(o=><option key={typeof o==="string"?o:o.v} value={typeof o==="string"?o:o.v}>{typeof o==="string"?o:o.l}</option>)}
          </select>
        </div>
      ))}
      <div style={{marginBottom:12}}><Label>Finish / color</Label><input type="text" value={c.finish} onChange={e=>update(c.id,{finish:e.target.value})} style={{...IS,width:"100%"}}/></div>
      <div style={{marginBottom:12}}>
        <Label>Wall</Label>
        <select value={c.wall} onChange={e=>update(c.id,{wall:e.target.value})} style={IS}>
          {[...activeWalls,"Island"].map(w=><option key={w} value={w}>{w}</option>)}
        </select>
      </div>
      <div style={{marginBottom:16}}><Label>Notes</Label><input type="text" value={c.notes} onChange={e=>update(c.id,{notes:e.target.value})} style={{...IS,width:"100%"}}/></div>
...
function QuoteView({cabs,project,setProject}) {
  ...
  const IS={background:"#fff",border:`1.5px solid ${T.border}`,borderRadius:6,padding:"7px 10px",fontSize:14,color:T.ink,outline:"none"};
  return (
          ...
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            {[["Project name","name"],["Client","client"]].map(([l,k])=>(
              <div key={k} style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <input value={project[k]} onChange={e=>setProject(p=>({...p,[k]:e.target.value}))} style={{...IS,width:220}}/>
              </div>
            ))}
          </div>
        </div>
            ...
            <Card style={{marginBottom:24,padding:0,overflow:"hidden"}}>
              <table style={{width:"100%",borderCollapse:"collapse",fontSize:14}}>
                <thead><tr style={{background:T.surfaceAlt,borderBottom:`1px solid ${T.border}`}}>
                  {["#","Type","W × H × D","Wall","Material","Door Style","Price"].map(h=><th key={h} style={{padding:"12px 14px",textAlign:"left",fontSize:12,fontWeight:600,color:T.muted,letterSpacing:"0.05em",textTransform:"uppercase"}}>{h}</th>)}
                </tr></thead>
                <tbody>{cabs.map((c,i)=>(
                  <tr key={c.id} style={{borderBottom:`1px solid ${T.border}`,background:i%2===0?"#fff":T.surface}}>
                    ...
                    <td style={{padding:"11px 14px",fontWeight:500}}>{DEFS[c.type]?.label}</td>
            ...
            <div style={{display:"flex",justifyContent:"flex-end",marginBottom:24}}>
              <Card style={{width:300,padding:20}}>
                {[["Subtotal",sub],["Tax (8%)",tax]].map(([l,v])=>(
                  <div key={l} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:`1px solid ${T.border}`,fontSize:15}}>
                    <span style={{color:T.muted}}>{l}</span><span>${v.toLocaleString()}</span>
                  </div>
                ))}
    ...
    <div style={{flex:1,overflowY:"auto",padding:"36px 48px",maxWidth:900}}>
      <div className="fade-up">
        <div style={{textAlign:"center",marginBottom:32}}>
          ...
          <p style={{fontSize:14,color:T.muted}}>{project.name} — {project.client||"No client"} — {new Date().toLocaleDateString()}</p>
        </div>
        <Card style={{marginBottom:24}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:16}}>
            {[["Project",project.name||"—"],["Client",project.client||"—"],["Room",`${(room.width/12).toFixed(1)}' × ${(room.depth/12).toFixed(1)}'`],["Ceiling",`${room.height}"`]].map(([l,v])=>(
              <div key={l}><Label>{l}</Label><div style={{fontSize:15,fontWeight:500}}>{v}</div></div>
            ))}
          </div>
        </Card>
            ...
            <div key={w} style={{marginBottom:28}}>
              ...
              <Card style={{padding:0,overflow:"hidden"}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                  <thead><tr style={{background:T.surfaceAlt,borderBottom:`1px solid ${T.border}`}}>
                    {["#","Type","W","H","D","Material","Door Style","Finish","Notes"].map(h=><th key={h} style={{padding:"10px 12px",textAlign:"left",fontSize:11,fontWeight:600,color:T.muted,letterSpacing:"0.05em",textTransform:"uppercase"}}>{h}</th>)}
                  </tr></thead>
                  <tbody>{wc.map((c,i)=>(
                    <tr key={c.id} style={{borderBottom:`1px solid ${T.border}`,background:i%2===0?"#fff":T.surface}}>
...
export default function App() {
  ...
  const addCab=type=>{const c=makeCab(type,type==="island"?"Island":wall,cabs,wallW);setCabs(p=>[...p,c]);setSel(c.id);};

  const applyRecs=rcs=>{
    const newCabs=(rcs||[]).filter(r=>DEFS[r.type]&&(r.wall==="Island"||activeWalls.includes(r.wall))).map(r=>{
      const ww=["East","West"].includes(r.wall)?room.depth:room.width;
      return{id:uid(),type:r.type,w:clamp(r.w||DEFS[r.type].w,9,96),h:clamp(r.h||DEFS[r.type].h,12,96),
        d:clamp(r.d||DEFS[r.type].d,9,36),x:clamp(r.x||3,0,ww-(r.w||DEFS[r.type].w)),ix:0,iy:0,
        wall:r.wall||activeWalls[0],material:r.material||"maple",doorStyle:r.doorStyle||"Shaker",
        finish:r.finish||"Natural",notes:r.notes||"",useStandard:true};
    });
    setCabs(newCabs);
  };
  ...
  const moveCab=(id,x)=>setCabs(p=>p.map(c=>c.id===id?{...c,x:clamp(x,0,wallW-c.w)}:c));
  ...
  const STEPS=[{id:"setup",n:"01",label:"Room setup"},{id:"recs",n:"02",label:"AI recommendations"},{id:"design",n:"03",label:"Design"},{id:"quote",n:"04",label:"Quote"},{id:"order",n:"05",label:"Order sheet"}];
    ...
    <div style={{height:"100vh",display:"flex",flexDirection:"column",background:T.bg,overflow:"hidden"}}>
      ...
      <div style={{background:"#fff",borderBottom:`1px solid ${T.border}`,height:58,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 24px",boxShadow:"0 1px 4px rgba(44,31,14,0.06)"}}>
        ...
        <nav style={{display:"flex",gap:2,height:"100%",alignItems:"stretch"}}>
          {STEPS.map(s=>{
            const isActive=view===s.id,isDone=STEPS.findIndex(x=>x.id===view)>STEPS.findIndex(x=>x.id===s.id);
            return <button key={s.id} onClick={()=>setView(s.id)}
              style={{padding:"0 16px",background:"transparent",border:"none",borderBottom:`3px solid ${isActive?T.amber:"transparent"}`,color:isActive?T.amber:isDone?T.oak:T.faint,fontSize:13,fontWeight:isActive?600:400,cursor:"pointer",display:"flex",alignItems:"center",gap:7,transition:"all 0.15s",whiteSpace:"nowrap"}}>
              <span style={{fontSize:11,fontWeight:700,opacity:0.6}}>{s.n}</span>{s.label}
            </button>;
          })}
        </nav>
        ...
        {view==="design"&&<>
          ...
          <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden",background:T.bg}}>
            <div style={{background:"#fff",borderBottom:`1px solid ${T.border}`,padding:"0 20px",display:"flex",alignItems:"stretch",flexShrink:0,height:44}}>
              {[["elevation","Elevation view"],["floorplan","Floor plan"]].map(([v,l])=>(
                <button key={v} onClick={()=>setSubView(v)}
                  style={{padding:"0 16px",background:"none",border:"none",borderBottom:`3px solid ${subView===v?T.amber:"transparent"}`,color:subView===v?T.amber:T.muted,fontSize:13,fontWeight:subView===v?600:400,cursor:"pointer",transition:"all 0.15s"}}>{l}</button>
              ))}
              <div style={{flex:1}}/>
            ...
            <div style={{flex:1,overflow:"auto",padding:28}}>
              {subView==="elevation"
                ?<ElevationCanvas cabs={cabs} wall={wall} wallW={wallW} wallH={room.height} sel={sel} onSel={setSel} onMove={moveCab}/>
